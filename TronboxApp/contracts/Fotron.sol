...
contract FotronCommon {
    
    //main adrministrators of the Fotron network
    mapping(address => bool) private _administrators;

    //main managers of the Fotron network
...
// Core contract for Fotron network
contract FotronCore is FotronCommon {
    ...
    uint256 constant public MIN_TOKEN_DEAL_VAL = 0.1 trx;
    uint256 constant public MAX_TOKEN_DEAL_VAL = 1000000 trx;

    // same same for TRX
    uint256 constant public MIN_TRX_DEAL_VAL = 0.001 trx;
    uint256 constant public MAX_TRX_DEAL_VAL = 200000 trx;
    
    // percent of a transaction commission which is taken for Big Promo bonus
    uint256 public _bigPromoPercent = 5 trx;

    // percent of a transaction commission which is taken for Quick Promo bonus
    uint256 public _quickPromoPercent = 5 trx;

    // percent of a transaction commission which is taken for Fotron DEV team
    uint256 public _devRewardPercent = 15 trx;
    
    // percent of a transaction commission which is taken for Token Owner. 
    uint256 public _tokenOwnerRewardPercent = 30 trx;
    ...
    uint256 public _shareRewardPercent = 25 trx;
    ...
    uint256 public _refBonusPercent = 20 trx;

    // interval of blocks for Big Promo bonus. It means that a user which buy a bunch of tokens for X TRX in that particular block will receive a special bonus 
    uint128 public _bigPromoBlockInterval = 9999;

    // same same for Quick Promo
    uint128 public _quickPromoBlockInterval = 100;
    
    // minimum trx amount of a purchase which is required to participate in promo.
    uint256 public _promoMinPurchaseTrx = 1 trx;
    
    // minimum trx purchase which is required to get a referal link.
    uint256 public _minRefTrxPurchase = 0.5 trx;

    // percent of fee which is supposed to distribute.
    uint256 public _totalIncomeFeePercent = 100 trx;
    ...
    //user saldo between buys and sels in trx per data contracts
    mapping(address => mapping(address => uint256)) private _userTrxVolumeSaldos;  
    ...
    //total volume trx per data contracts
    mapping(address => uint256) private _totalVolumeTrx;
    //total volume tokens per data contracts
    mapping(address => uint256) private _totalVolumeToken;

    
    event onWithdrawUserBonus(address indexed userAddress, uint256 trxWithdrawn); 
    ...
    constructor() FotronCommon() public { 
    ...
    function setPromoMinPurchaseTrx(uint256 val) onlyAdministrator public {
        _promoMinPurchaseTrx = val;
    }
    
    function setMinRefTrxPurchase(uint256 val) onlyAdministrator public {
        _minRefTrxPurchase = val;
    }
    
    function setTotalIncomeFeePercent(uint256 val) onlyController public {
        require(val > 0 && val <= 100 trx);
    ...
    function setRewardPercentages(uint256 tokenOwnerRewardPercent, uint256 shareRewardPercent, uint256 refBonusPercent, uint256 bigPromoPercent, uint256 quickPromoPercent) onlyAdministrator public {
        require(tokenOwnerRewardPercent <= 40 trx);
        require(shareRewardPercent <= 100 trx);
        require(refBonusPercent <= 100 trx);
        require(bigPromoPercent <= 100 trx);
        require(quickPromoPercent <= 100 trx);

        require(tokenOwnerRewardPercent + shareRewardPercent + refBonusPercent + _devRewardPercent + _bigPromoPercent + _quickPromoPercent == 100 trx);
    ...
    function payoutQuickBonus(address userAddress) onlyController public {
        address dataContractAddress = Fotron(msg.sender).getDataContractAddress();
    ...
    function getTotalBonusPerShare() public view returns (uint256 res) {
        for (uint256 i = 0; i < _controllerContractCount; i++) {
            res = SafeMath.add(res, _bonusesPerShare[Fotron(_controllerIndexer[i]).getDataContractAddress()]);
        }          
    }
    
    
    function addBonusPerShare() onlyController payable public {
        FotronData data = Fotron(msg.sender)._data();
    ...
    function resetUserRefBalance(address userAddress) onlyController public {
        resetUserRefBalance(Fotron(msg.sender).getDataContractAddress(), userAddress);
    }
    ...
    function addUserRewardPayouts(address userAddress, uint256 val) onlyController public {
        addUserRewardPayouts(Fotron(msg.sender).getDataContractAddress(), userAddress, val);
    }    
    ...
    function resetUserPromoBonus(address userAddress) onlyController public {
        resetUserPromoBonus(Fotron(msg.sender).getDataContractAddress(), userAddress);
    }
    ...
    function trackBuy(address userAddress, uint256 volTrx, uint256 volToken) onlyController public {
        address dataContractAddress = Fotron(msg.sender).getDataContractAddress();
        _buyCounts[dataContractAddress] = SafeMath.add(_buyCounts[dataContractAddress], 1);
        _userTrxVolumeSaldos[dataContractAddress][userAddress] = SafeMath.add(_userTrxVolumeSaldos[dataContractAddress][userAddress], volTrx);
        
        trackTotalVolume(dataContractAddress, volTrx, volToken);
    }

    function trackSell(address userAddress, uint256 volTrx, uint256 volToken) onlyController public {
        address dataContractAddress = Fotron(msg.sender).getDataContractAddress();
        _sellCounts[dataContractAddress] = SafeMath.add(_sellCounts[dataContractAddress], 1);
        _userTrxVolumeSaldos[dataContractAddress][userAddress] = SafeMath.sub(_userTrxVolumeSaldos[dataContractAddress][userAddress], volTrx);
        
        trackTotalVolume(dataContractAddress, volTrx, volToken);
    }
    
    function trackTotalVolume(address dataContractAddress, uint256 volTrx, uint256 volToken) internal {
        _totalVolumeTrx[dataContractAddress] = SafeMath.add(_totalVolumeTrx[dataContractAddress], volTrx);
    ...
    function getTotalBuyCount() public view returns (uint256 res) {
        for (uint256 i = 0; i < _controllerContractCount; i++) {
            res = SafeMath.add(res, _buyCounts[Fotron(_controllerIndexer[i]).getDataContractAddress()]);
        }         
    }
    ...
    function getTotalSellCount() public view returns (uint256 res) {
        for (uint256 i = 0; i < _controllerContractCount; i++) {
            res = SafeMath.add(res, _sellCounts[Fotron(_controllerIndexer[i]).getDataContractAddress()]);
        }         
    }

    function getTotalVolumeTrx(address dataContractAddress) public view returns (uint256) {
        return _totalVolumeTrx[dataContractAddress];
    }
    ...
    function getUserTrxVolumeSaldo(address dataContractAddress, address userAddress) public view returns (uint256) {
        return _userTrxVolumeSaldos[dataContractAddress][userAddress];
    }
    
    function getUserTotalTrxVolumeSaldo(address userAddress) public view returns (uint256 res) {
        for (uint256 i = 0; i < _controllerContractCount; i++) {
            res = SafeMath.add(res, _userTrxVolumeSaldos[Fotron(_controllerIndexer[i]).getDataContractAddress()][userAddress]);
        } 
    }
    ...
    function getUserReward(address dataContractAddress, address userAddress, bool incShareBonus, bool incRefBonus, bool incPromoBonus) public view returns(uint256 reward) {
        FotronData data = FotronData(dataContractAddress);
    ...
    function getUserTotalReward(address userAddress, bool incShareBonus, bool incRefBonus, bool incPromoBonus) public view returns(uint256 res) {
        for (uint256 i = 0; i < _controllerContractCount; i++) {
            address dataContractAddress = Fotron(_controllerIndexer[i]).getDataContractAddress();
    ...
    function isRefAvailable(address refAddress) public view returns(bool) {
        return getUserTotalTrxVolumeSaldo(refAddress) >= _minRefTrxPurchase;
    }
    ...
    //withdaw all the remamining TRX if there is no one active contract. We don't want to leave them here forever
    function withdrawRemainingTrxAfterAll() onlyAdministrator public {
        for (uint256 i = 0; i < _controllerContractCount; i++) {
            if (Fotron(_controllerIndexer[i]).isActive()) revert();
        }
    ...
    function calcPercent(uint256 amount, uint256 percent) public pure returns(uint256) {
        return SafeMath.div(SafeMath.mul(SafeMath.div(amount, 100), percent), 1 trx);
    }
...
contract FotronData {
    ...
    function getPromoMinPurchaseTrx() public view returns(uint256) {
        return _core._promoMinPurchaseTrx();
    }
    ...
    function getUserTrxVolumeSaldo(address userAddress) public view returns(uint256) {
        return _core.getUserTrxVolumeSaldo(address(this), userAddress);
    }
...
contract Fotron {
    ...
    event onTokenPurchase(address indexed userAddress, uint256 incomingTrx, uint256 tokensMinted, address indexed referredBy);
    
    event onTokenSell(address indexed userAddress, uint256 tokensBurned, uint256 trxEarned);
    
    event onReinvestment(address indexed userAddress, uint256 trxReinvested, uint256 tokensMinted);
    
    event onWithdrawTokenOwnerReward(address indexed toAddress, uint256 trxWithdrawn); 

    event onWinQuickPromo(address indexed userAddress, uint256 trxWon);    
   
    event onWinBigPromo(address indexed userAddress, uint256 trxWon);    
    ...
    // trx value must be greater than 0 for purchase transactions
    ...
    constructor(address tokenContractAddress, address dataContractAddress) public {
        ...
        _data = FotronData(dataContractAddress);
    ...
    function finish() onlyActive onlyAdministrator public {
        ...
        msg.sender.transfer(getTotalTrxBalance());
        
        isActive = false;
    }
    
    //Converts incoming trx to tokens
    function buy(address refAddress, uint256 minReturn) onlyActive validPayableValue public payable returns(uint256) {
        return purchaseTokens(msg.value, refAddress, minReturn);
    }

    //sell tokens for trx. before call this func you have to call "approve" in the ERC20 token contract
    function sell(uint256 tokenAmount, uint256 minReturn) onlyActive onlyContractUsers public returns(uint256) {
        ...
        uint256 trxAmount = 0; uint256 totalFeeTrx = 0; uint256 tokenPrice = 0;
        (trxAmount, totalFeeTrx, tokenPrice) = estimateSellOrder(tokenAmount, true);
        require(trxAmount >= minReturn);
        ...
        distributeFee(totalFeeTrx, address(0x0));

        _core.trackSell(msg.sender, trxAmount, tokenAmount);
       
        emit onTokenSell(msg.sender, tokenAmount, trxAmount);

        return trxAmount;
    }   


    //Fallback function to handle trx that was sent straight to the contract
    function() onlyActive validPayableValue payable external {
    ...
    // request migration to new contract. After request Fotron dev team should review its code and approve it if it is OK
    ...
    //migrate to new controller contract in case of some mistake in the contract and transfer there all the tokens and trx. It can be done only after code review by Fotron developers.
    ...
    function getBonusPerShare() public view returns (uint256) {
        return SafeMath.div(SafeMath.mul(_data.getBonusPerShare(), 1 trx), _core.MAGNITUDE());
    }    
    //token initial price in TRX
    ...
    function getMinRefTrxPurchase() public view returns (uint256) {
        return _core._minRefTrxPurchase();
    }    
    ...
    //contract's trx balance
    function getTotalTrxBalance() public view returns(uint256) {
    ...
    //max and min values of a deal in TRX
    function getTrxDealRange() public view returns(uint256, uint256) {
        uint256 minTokenVal; uint256 maxTokenVal;
        (minTokenVal, maxTokenVal) = getTokenDealRange();
        
        return ( SafeMath.max(_core.MIN_TRX_DEAL_VAL(), tokensToTrx(minTokenVal, true)), SafeMath.min(_core.MAX_TRX_DEAL_VAL(), tokensToTrx(maxTokenVal, true)) );
    }
    ...
    function get1TokenBuyPrice() public view returns(uint256) {
        ...
        return SafeMath.div(trxAmount * 1 trx, tokenAmount);
    }
    ...
    //esimate buy order by amount of TRX/tokens. returns tokens/trx amount after the deal, total fee in TRX and average token price
    function estimateBuyOrder(uint256 amount, bool fromTrx) public view returns(uint256, uint256, uint256) {
        uint256 minAmount; uint256 maxAmount;
        (minAmount, maxAmount) = fromTrx ? getTrxDealRange() : getTokenDealRange();
        //require(amount >= minAmount && amount <= maxAmount);

        uint256 trxAmount = fromTrx ? amount : tokensToTrx(amount, true);
        require(trxAmount > 0);
        ...
        uint256 tokenPrice = SafeMath.div(trxAmount * 1 trx, tokenAmount);

        return (fromTrx ? tokenAmount : SafeMath.add(trxAmount, totalFeeTrx), totalFeeTrx, tokenPrice);
    }
    
    //esimate sell order by amount of tokens/TRX. returns trx/tokens amount after the deal, total fee in TRX and average token price
    function estimateSellOrder(uint256 amount, bool fromToken) public view returns(uint256, uint256, uint256) {
        ...
        uint256 trxAmount = fromToken ? tokensToTrx(tokenAmount, false) : amount;
        uint256 totalFeeTrx = calcTotalFee(tokenAmount, false);
        require(trxAmount > totalFeeTrx);

        uint256 tokenPrice = SafeMath.div(trxAmount * 1 trx, tokenAmount);
        
        return (fromToken ? trxAmount : tokenAmount, totalFeeTrx, tokenPrice);
    }
    ...
    function checkAndSendPromoBonus(uint256 purchaseAmountTrx) internal {
        if (purchaseAmountTrx < _data.getPromoMinPurchaseTrx()) return;
    ...
    function distributeFee(uint256 totalFeeTrx, address refAddress) internal {
        addProfitPerShare(totalFeeTrx, refAddress);
        addDevReward(totalFeeTrx);
        addTokenOwnerReward(totalFeeTrx);
        addBigPromoBonus(totalFeeTrx);
        addQuickPromoBonus(totalFeeTrx);
    }

    function addProfitPerShare(uint256 totalFeeTrx, address refAddress) internal {
        uint256 refBonus = calcRefBonus(totalFeeTrx);
        uint256 totalShareReward = calcTotalShareRewardFee(totalFeeTrx);
    ...
    function addTokenOwnerReward(uint256 totalFeeTrx) internal {
        _data.addTokenOwnerReward(calcTokenOwnerReward(totalFeeTrx));
    }  

    function addBigPromoBonus(uint256 totalFeeTrx) internal {
        _core.addBigPromoBonus.value(calcBigPromoBonus(totalFeeTrx))();
    }

    function addQuickPromoBonus(uint256 totalFeeTrx) internal {
        _core.addQuickPromoBonus.value(calcQuickPromoBonus(totalFeeTrx))();
    }   
    ...
    function tokensToTrx(uint256 tokenAmount, bool isBuy) internal view returns(uint256) {
        ...
        int128 realTrxAmountFor1Token = RealMath.mul(_data._realTokenPrice(), RealMath.exp(expArg));
        int128 realTrxAmount = RealMath.mul(realTokenAmount, realTrxAmountFor1Token);

        return _core.convertRealTo256(realTrxAmount);
    }
